...
public class MealsUtil {
    public static CopyOnWriteArrayList<Meal> mealsC = new CopyOnWriteArrayList<>();
    ...
    //public static List<MealWithExceed> mealsE = MealsUtil.getWithExceeded(meals,2000);
    static{
        meals.stream().forEach(x -> mealsC.add(x));
    }
    public static void main(String[] args) {
        ...
        meals.stream().map(m -> m.getCount()).forEach(System.out::println);
    }
...
//                      Collectors.toMap(Meal::getDate, Meal::getCalories, Integer::sum)
    ...
    public static MealWithExceed createWithExceed(Meal meal, boolean exceeded) {
        MealWithExceed me = new MealWithExceed(meal.getDateTime(), meal.getDescription(), meal.getCalories(), exceeded);
        me.setCount(meal.getCount());
        return  me;
    }
}